...
def test_client_config_file_doesnt_exist(mock_config_path):
    ...
    mocklog.info.assert_any_call('Config file mariadb_config.json at ./mariadb_config.json does not exist')
...
def test_client_config_invalid_json(mock_config_path):
    ...
    mocklog.info.assert_any_call('Config file mariadb_config.json at ./mariadb_config.json is not valid JSON: '
...
def test_client_config_unsupported_options(mock_config_path):
    ...
    mocklog.info.assert_any_call('Config file mariadb_config.json at ./mariadb_config.json '
...
def test_client_config_loads_correct_values(mock_config_path):
    ...
    mocklog.info.assert_called_once_with(f'Loading config file at ./mariadb_config.json...')

    assert cfg.get_args() == '--user=testuser --host=testhost --port=00 --password=secret_password '
...
def test_client_config_path():
    ...
    with patch.dict('os.environ', {'JUPYTER_CONFIG_DIR': ''}):
        assert cfg._config_path() == os.path.join(os.path.expanduser("~"), ".jupyter/mariadb_config.json")
